      ...
      <v-progress-linear color="pink" class="processing-el" v-show="loadingUserInfo" v-bind:indeterminate="true"></v-progress-linear>
        ...
        <v-list two-line v-show="items.length > 0">
          <template v-for="item in items">
            ...
            <v-list-tile height-auto avatar v-else v-bind:key="item.title" @click="" class="height-auto">
              <v-list-tile-content>
                <v-layout flex row wrap class="w-100">
                  <v-flex xs12 sm10>
                  ...
                  <v-flex xs12 sm2 p0 class="text-right">
                    <v-btn flat icon color="grey darken-1" @click="deleteWeight(item._id)">
...
export default {
  ...
  methods: {
    saveWeight: function() {
      ...
      Rest.routes.weight.create({
        ...
        if (data.errors[0].type == 'ResourceExistsError'){
          ...
          _self.updateStatusMessage = 'Weight resource exists';
          // _self.updateButton = 'Yes update it!';
          // _self.updateConfirm = function() {
          //     _self.updating = true;
          //     _self.updateStatusMessage = 'Working on update';
          //     Rest.routes.weight.update({
          //       params:{_id: _id},
          //       data: data,
          //     }).then(function(m) {
          //       console.log(m)
    ...
    deleteWeight: function(_id) {
      this.updateDialog = true;
      this.updateTitle = 'Delete Weight Entry';
